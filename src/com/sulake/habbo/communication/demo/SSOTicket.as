package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      private var var_1053:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String)
      {
         ...
         _assets = param1;
         var_1053 = param2;
         var_1052 = "http://" + param3 + "/client";
         var_1053.init();
         var_1053.addEventListener(HabboWeb.const_215,requestClientURL);
      }
      ...
      {
         var _loc2_:String = var_1052;
         if(_assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + var_1052);
         }
         var _loc3_:URLRequest = new URLRequest(var_1052);
         if(_assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            var_1053.requestReAuthenticate();
         }
         else
         {
            try
            {
               var_1307 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               var_1572 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               if(false)
               {
