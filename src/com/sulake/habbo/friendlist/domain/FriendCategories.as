package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_454).received = true;
         findCategory(FriendCategory.const_904).received = true;
      ...
      private function removeFriend(param1:int, param2:Boolean) : Boolean
      {
         ...
         if(param2)
         {
            var_1023[param1] = null;
         }
         var _loc3_:Boolean = false;
         for each(_loc4_ in var_326)
         {
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_397.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_397.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_397.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_397.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_397.view.setNewMessageArrived();
               var_397.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
      ...
      {
         return var_1023[param1];
      }
      
      private function flushReceivedStatus() : void
      {
         var _loc1_:* = null;
         for each(_loc1_ in var_326)
         {
