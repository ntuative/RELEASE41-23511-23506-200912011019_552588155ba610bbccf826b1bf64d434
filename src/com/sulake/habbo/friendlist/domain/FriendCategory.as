package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         return Math.ceil(this.var_234.length / const_322);
      }
      ...
      {
         if(this.var_834 >= this.getPageCount())
         {
            this.var_834 = Math.max(0,this.getPageCount() - 1);
         }
      }
      ...
      public function addFriend(param1:Friend) : void
      {
         ...
         {
            _loc4_ = var_234[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               var_234.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         checkPageIndex();
         return Math.min((this.var_834 + 1) * const_322,this.var_234.length);
      }
